...
const PageHero = ({ title, subtitle, image }: PageHeroProps) => (
  <section className="relative bg-industrial overflow-hidden min-h-[280px] md:min-h-[340px] flex items-center">
    {image && (
    ...
    <div className="relative container mx-auto px-4 lg:px-8 pt-20 pb-24 md:pt-24 md:pb-28">
      <motion.h1
        initial={{ opacity: 0, y: 30 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6 }}
        className="font-heading text-3xl md:text-5xl font-bold text-industrial-foreground mb-4"
      >
        {title}
      </motion.h1>
      {subtitle && (
        <motion.p
          ...
          className="text-lg md:text-xl text-industrial-foreground/70 max-w-2xl"
        >
          {subtitle}
        </motion.p>
      )}
